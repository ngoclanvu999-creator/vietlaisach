...
Đầu ra luôn là BỘ BỐN PHẦN chuẩn 2025. Thang điểm Phần II lũy tiến
0,1 / 0,25 / 0,5 / 1,0 theo số ý đúng — KHÔNG phải 0,25 nhân số ý. Đây là chỗ
dễ lập trình sai nhất; `core/loai_dau_ra.kiem_tra_tong_diem()` canh nó.

Chạy:  python -m uvicorn app_de_thi.main:app --port 8502
"""
...
from core.loai_dau_ra import QUY_CACH, quy_cach, thu_muc_cua, tinh_cau_truc
...
def doc_cai_dat():
    ...
    ra["loai_dau_ra"] = [{"ma": m, "ten": QUY_CACH[m].ten} for m in CH.LOAI_DAU_RA]
    return ra
...
@app.get("/api/cau-truc-de")
def cau_truc_de(loai: str = "", mon: str = "toan"):
    """
    Cho người dùng xem TRƯỚC đề sẽ gồm bao nhiêu câu, mỗi phần mấy điểm.

    Biết trước cấu trúc thì mới biết tài liệu gốc có đủ câu hay không — nạp 12
    câu mà chọn đề học kì 22 câu thì phần còn lại sẽ rỗng.
    """
    ma = loai if loai in CH.LOAI_DAU_RA else CH.LOAI_MAC_DINH
    ct = tinh_cau_truc(ma, mon if mon in ("toan", "vatly") else "toan")
    qc = quy_cach(ma)
    return {
        "ma": ma,
        "ten": qc.ten,
        "thoi_gian": qc.thoi_gian,
        "pham_vi": qc.pham_vi,
        "phan_1": {"so_cau": ct.so_cau_p1,
                   "diem_moi_cau": ct.diem_moi_cau_p1,
                   "diem": round(ct.so_cau_p1 * ct.diem_moi_cau_p1, 2)},
        "phan_2": {"so_cau": ct.so_cau_p2,
                   "diem_toi_da_moi_cau": ct.diem_toi_da_p2,
                   "luy_tien": ct.luy_tien_p2,
                   "diem": round(ct.so_cau_p2 * ct.diem_toi_da_p2, 2)},
        "phan_3": {"so_cau": ct.so_cau_p3,
                   "diem_moi_cau": ct.diem_moi_cau_p3,
                   "diem": round(ct.so_cau_p3 * ct.diem_moi_cau_p3, 2)},
        "tong_cau": ct.tong_cau,
        "tong_diem": round(ct.so_cau_p1 * ct.diem_moi_cau_p1
                           + ct.so_cau_p2 * ct.diem_toi_da_p2
                           + ct.so_cau_p3 * ct.diem_moi_cau_p3, 2),
    }
...
def bien_soan(
    ...
    loai = loai_dau_ra if loai_dau_ra in CH.LOAI_DAU_RA else CH.LOAI_MAC_DINH
    api_key, model_name = khoa_cua_yeu_cau(request)
    ...
    # Đề thi KHÔNG chen mẹo Casio và cảnh báo bẫy vào giữa đề — học sinh nhìn
    # xuống là thấy gợi ý, mất hẳn tác dụng kiểm tra.
    tuy_chon = {
        "solution": _co(solution), "answer_key": _co(answer_key),
        "casio": False, "traps": False, "theory": False,
    }

    try:
        book = process_rewrite_pipeline(
            questions=cau_hoi, subject=subject, add_count=0,
        ...
        than = "".join(c for c in book.new_title[:30]
                       if c.isalnum() or c in " -_").strip().replace(" ", "_")
        tien_to = quy_cach(loai).ten.replace(" ", "_")
        ten_ra = f"{tien_to}_{than or 'Tai_lieu'}.docx"

        thu_muc = CH.THU_MUC_RA / thu_muc_cua(loai)
        duong_ra = thu_muc / ten_ra
        ...
        return {
            ...
            "ten_tep": ten_ra,
            "download_url": f"/api/tai-ve/{thu_muc_cua(loai)}/{ten_ra}",
        }
    except Exception as e:
        import traceback
        traceback.print_exc()
