/**
 * Giao diện app SOẠN ĐỀ THI.
 *
 * Năm loại đề dùng chung một bộ dựng và một chuẩn nghiệp vụ, chỉ khác thời
 * gian làm bài và số câu. Nên app này thực chất là MỘT việc, năm biến thể.
 *
 * Điểm khác app chuyên đề: có ô xem trước CẤU TRÚC ĐỀ. Biết trước đề gồm bao
 * nhiêu câu mới biết tài liệu gốc có đủ hay không — nạp 12 câu mà chọn đề học
 * kì 22 câu thì phần còn lại sẽ rỗng.
 *
...
(function () {
    ...
    document.querySelectorAll(".radio-card").forEach((the) => {
        the.addEventListener("click", () => {
            ...
            veCauTrucDe();      // Toán và Vật lí có số câu khác nhau
        });
    });
    ...
    // ======================================================================
    // BƯỚC 2 — xem trước cấu trúc đề
    // ======================================================================
    async function veCauTrucDe() {
        const o = $("bang-cau-truc");
        o.textContent = "Đang tính…";
        try {
            const r = await fetch("/api/cau-truc-de?loai=" + $("loai-dau-ra").value
                                  + "&mon=" + monDangChon());
            const d = await r.json();
            const p2 = d.phan_2;
            o.innerHTML =
                "<strong>" + d.tong_cau + " câu</strong> · " + d.thoi_gian + " phút · "
                + "tổng <strong>" + d.tong_diem.toFixed(2) + " điểm</strong><br>"
                + "Phần I: " + d.phan_1.so_cau + " câu trắc nghiệm — "
                + d.phan_1.diem.toFixed(2) + " điểm<br>"
                + "Phần II: " + p2.so_cau + " câu đúng/sai — " + p2.diem.toFixed(2)
                + " điểm (lũy tiến " + (p2.luy_tien || []).join(" / ") + ")<br>"
                + "Phần III: " + d.phan_3.so_cau + " câu trả lời ngắn — "
                + d.phan_3.diem.toFixed(2) + " điểm<br>"
                + '<span class="field-hint">' + thoat(d.pham_vi) + "</span>";
            canhBaoThieuCau(d.tong_cau);
        } catch (e) {
            o.textContent = "Không lấy được cấu trúc đề: " + e.message;
        }
    }

    // Nạp ít câu hơn đề yêu cầu thì phần còn lại sẽ rỗng — nói trước, đừng để
    // người dùng dựng xong mới thấy đề thiếu bài.
    let soCauDaNap = 0;
    function canhBaoThieuCau(canCo) {
        const o = $("bang-cau-truc");
        if (soCauDaNap && canCo && soCauDaNap < canCo) {
            o.innerHTML += '<div class="doctype-warn">⚠️ Tài liệu gốc mới có '
                + soCauDaNap + " câu, đề này cần " + canCo
                + " câu. Phần thiếu sẽ để trống.</div>";
        }
    }

    $("loai-dau-ra").addEventListener("change", veCauTrucDe);
    ...
    $("btn-bien-soan").addEventListener("click", async () => {
        ...
        fd.append("loai_dau_ra", $("loai-dau-ra").value);
        fd.append("subject", monDangChon());
        ...
        fd.append("answer_key", $("co-bang-dap-an").checked ? "1" : "0");
