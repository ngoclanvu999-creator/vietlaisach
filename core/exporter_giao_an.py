...
def xuat_giao_an(doc, book, subject: str = "toan", cap_hoc: str = "THPT",
    ...
    _p(doc, "1. Về kiến thức", bold=True, size=12, thut=12, after=2)
    _cho_dien(doc, "Nêu yêu cầu cần đạt của bài này theo chương trình GDPT 2018.")

    _p(doc, "2. Về năng lực", bold=True, size=12, thut=12, before=6, after=2)
    ...
    _p(doc, "3. Về phẩm chất", bold=True, size=12, thut=12, before=6, after=2)
    ...
    _bon_muc(
        ...
        "Giáo viên nêu tình huống → học sinh suy nghĩ cá nhân 1–2 phút → "
        "một vài học sinh phát biểu → giáo viên dẫn vào bài.",
    )

    # Hoạt động 2 — khung
    _muc_hoat_dong(doc, 2, "Hình thành kiến thức mới")
    _bon_muc(
        doc,
        ...
        "Giáo viên tổ chức cho học sinh đọc sách giáo khoa và trả lời câu hỏi "
        "dẫn dắt → thảo luận cặp đôi → giáo viên chốt kiến thức.",
    )
    ...
    _bon_muc(
        ...
        "Học sinh làm cá nhân → đổi vở kiểm tra chéo → giáo viên chữa bài và "
        "nhấn mạnh sai lầm thường gặp.",
    )
    ...
    _bon_muc(
        ...
        "Giao về nhà hoặc làm theo nhóm tại lớp nếu còn thời gian → "
        "báo cáo ở tiết sau.",
    )
